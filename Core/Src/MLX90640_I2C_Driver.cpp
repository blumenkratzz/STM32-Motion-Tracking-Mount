...
int MLX90640_I2CRead(I2C_HandleTypeDef *hi2c, uint8_t slaveAddr, uint16_t startAddress, uint16_t nMemAddressRead, uint16_t *data)
{
    ...
    ack = HAL_I2C_Mem_Read(hi2c, (slaveAddr<<1), startAddress, I2C_MEMADD_SIZE_16BIT, bp, nMemAddressRead*2, 500);
...
int MLX90640_I2CWrite(I2C_HandleTypeDef *hi2c,uint8_t slaveAddr, uint16_t writeAddress, uint16_t data)
{
    ...
    ack = HAL_I2C_Mem_Write(hi2c, sa, writeAddress, I2C_MEMADD_SIZE_16BIT, cmd, sizeof(cmd), 500);
    ...
    MLX90640_I2CRead(hi2c,slaveAddr,writeAddress,1, &dataCheck);
