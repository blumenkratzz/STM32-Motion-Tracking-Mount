...
{
     return MLX90640_I2CRead(hi2c, slaveAddr, 0x2400, 832, eeData);
}
...
int MLX90640_GetFrameData(I2C_HandleTypeDef *hi2c, uint8_t slaveAddr, uint16_t *frameData)
{
    ...
    while(dataReady == 0)
    {
        error = MLX90640_I2CRead(hi2c, slaveAddr, 0x8000, 1, &statusRegister);
        if(error != 0)
        {
            return error;
        }    
    ...
    { 
        error = MLX90640_I2CWrite(hi2c,slaveAddr, 0x8000, 0x0030);
        if(error == -1)
        {
            return error;
        }
            
        error = MLX90640_I2CRead(hi2c, slaveAddr, 0x0400, 832, frameData);
        if(error != 0)
        {
            return error;
        }
    ...
    error = MLX90640_I2CRead(hi2c, slaveAddr, 0x800D, 1, &controlRegister1);
...
int MLX90640_SetResolution(I2C_HandleTypeDef *hi2c, uint8_t slaveAddr, uint8_t resolution)
{
    ...
    if(error == 0)
    {
        ...
        error = MLX90640_I2CWrite(hi2c,slaveAddr, 0x800D, value);
    }    
